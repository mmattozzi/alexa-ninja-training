...
const main = {
  LaunchRequestHandler: {
    ...
    handle(handlerInput) {
      ...
      let response = handlerInput.responseBuilder
        .speak("Welcome to ninja training! Press a button to turn it into your ninja buzzer.")
        .getResponse();
  ...
  StopAndCancelIntentHandler: {
    ...
    handle(handlerInput) {
      ...
      var response = handlerInput.responseBuilder
        .speak("Thank you for training to be a ninja! Goodbye.")
        .getResponse();
        ...
        switch (gameEngineEvents[i].name) {
          case 'button_down_event': {
            ...
            if (sessionAttributes[buttonId + '_initialized'] === undefined) {
              ...
              if (sessionAttributes["redBuzzer"] === undefined) {
                  sessionAttributes["redBuzzer"] = buttonId;
              } else if (sessionAttributes["resetButton"] === undefined) {
                  sessionAttributes["resetButton"] = buttonId;
              }
            }
            ...
            if (isNewButton) {
              ...
              if (buttonId == sessionAttributes["redBuzzer"]) {
                  response = handlerInput.responseBuilder
                        .speak("Ok, now press the other button to become your reset button.")
                        .getResponse();
                        response.directives = [];
                response.directives.push(buildButtonIdleAnimationDirective([buttonId], breathAnimationRed));    
              } else if (buttonId == sessionAttributes["resetButton"]) {
                  response.directives = [];
                response.directives.push(buildButtonIdleAnimationDirective([buttonId], breathAnimationBlue));
                const signedStartUrl = Util.getS3PreSignedUrl("Media/ninja-start-timer-48.mp3");
                    response = handlerInput.responseBuilder
                        .speak("Great! Now it's time to start training. <audio src='" + signedStartUrl.encodeXML() + "'/>")
                        .getResponse();
              }
            } else { 
                if (buttonId == sessionAttributes["redBuzzer"]) {
                    const signedHornUrl = Util.getS3PreSignedUrl("Media/ninjahorn-48.mp3");
                    response = handlerInput.responseBuilder
                        .speak("<audio src='" + signedHornUrl.encodeXML() + "'/>")
                        .getResponse();
                    response.directives = [];
                } else if (buttonId == sessionAttributes["resetButton"]) {
                    const signedStartUrl = Util.getS3PreSignedUrl("Media/ninja-start-timer-48.mp3");
                    response = handlerInput.responseBuilder
                        .speak("<audio src='" + signedStartUrl.encodeXML() + "'/>")
                        .getResponse();
                    response.directives = [];
                }
            }
